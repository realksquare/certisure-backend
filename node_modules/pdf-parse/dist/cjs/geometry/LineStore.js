...
class LineStore {
    ...
    getTables() {
        ...
        while (this.hLines.length !== 0) {
            ...
            if (filled)
                continue;
            const table = new Table_1.Table(hLine);
        ...
        while (this.vLines.length !== 0) {
            ...
            if (filled)
                continue;
            const table = new Table_1.Table(vLine);
            this.fillTable(table);
            result.push(table);
        }
    ...
    normalizeHorizontal() {
        ...
        for (const line of this.hLines) {
            ...
            else if (Math.abs(sameY[0]?.from.y - line.from.y) < Shape_1.Shape.tolerance) {
                sameY.push(line);
            }
            else {
    ...
    normalizeVertical() {
        ...
        for (const line of this.vLines) {
            ...
            else if (Math.abs(sameX[0]?.from.x - line.from.x) < Shape_1.Shape.tolerance) {
                sameX.push(line);
            }
            else {
    ...
    margeHorizontalLines(sameYLines) {
        ...
        for (const line of sameYLines) {
            if (line.from.x - maxX < Shape_1.Shape.tolerance) {
                if (line.from.x < minX) {
                    minX = line.from.x;
                }
                if (line.to.x > maxX) {
                ...
                if (maxX > minX) {
                    result.push(new Line_1.Line(new Point_1.Point(minX, sameY), new Point_1.Point(maxX, sameY)));
                }
                minX = line.from.x;
                maxX = line.to.x;
            }
    ...
    margeVerticalLines(sameXLines) {
        ...
        for (const line of sameXLines) {
            if (line.from.y - maxY < Shape_1.Shape.tolerance) {
                if (line.from.y < minY) {
                    minY = line.from.y;
                }
                if (line.to.y > maxY) {
                ...
                if (maxY > minY) {
                    result.push(new Line_1.Line(new Point_1.Point(sameX, minY), new Point_1.Point(sameX, maxY)));
                }
                minY = line.from.y;
                maxY = line.to.y;
            }
