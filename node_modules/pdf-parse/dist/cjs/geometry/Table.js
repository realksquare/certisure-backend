...
class Table {
    ...
    vLines = [];
    constructor(line) {
        if (line.direction === Line_1.LineDirection.Horizontal) {
            this.hLines.push(line);
        }
        else if (line.direction === Line_1.LineDirection.Vertical) {
    ...
    mergeHorizontalLines(lines) {
        ...
        const resultLine = new Line_1.Line(new Point_1.Point(minX, lines[0].from.y), new Point_1.Point(maxX, lines[0].from.y));
        for (let i = 1; i < lines.length; i++) {
            const prevLine = lines[i - 1];
            const currLine = lines[i];
            if (Math.abs(prevLine.to.x - currLine.from.x) > Shape_1.Shape.tolerance) {
                const gapLine = new Line_1.Line(new Point_1.Point(prevLine.to.x, prevLine.from.y), new Point_1.Point(currLine.from.x, currLine.from.y));
    ...
    mergeVerticalLines(lines) {
        ...
        const resultLine = new Line_1.Line(new Point_1.Point(lines[0].from.x, minY), new Point_1.Point(lines[0].from.x, maxY));
        for (let i = 1; i < lines.length; i++) {
            const prevLine = lines[i - 1];
            const currLine = lines[i];
            if (Math.abs(prevLine.to.y - currLine.from.y) > Shape_1.Shape.tolerance) {
                const gapLine = new Line_1.Line(new Point_1.Point(prevLine.to.x, prevLine.to.y), new Point_1.Point(prevLine.to.x, currLine.from.y));
                resultLine.addGap(gapLine);
            }
        }
        return resultLine;
    ...
    verticalExists(line, y1, y2) {
        if (line.direction !== Line_1.LineDirection.Vertical) {
    ...
    getRow(h1Index, h2Index, yMiddle) {
        ...
        for (let i = 1; i < vLineIndexes.length; i++) {
            ...
            const tableCell = {
                minXY: new Point_1.Point(leftVLine.from.x, topHLine.from.y),
                maxXY: new Point_1.Point(rightVLine.from.x, bottomHLine.from.y),
    ...
    toData() {
        ...
        const minXY = new Point_1.Point(colPivots[0], rowPivots[0]);
        const maxXY = new Point_1.Point(colPivots[colPivots.length - 1], rowPivots[rowPivots.length - 1]);
        const result = new TableData_1.TableData(minXY, maxXY, rowPivots, colPivots);
